...
def parse_args():
    ...
    parser.add_argument('--input', '-i', 
                       default='files/key.csv',
                       help='Input CSV file with node IDs')
...
async def main():
    ...
    async with aiofiles.open(f"files\\{args.language}-{args.region}.csv", 'a', newline='') as file:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async for node_id in read(args.input):
                ...
                logging.info(f"Найдены продукты: {data['variants']}, аксесуары {data['product']} - [{node_id}]")
                if data['variants']:
                    page = await Pagination.create(node_id, sie, SieportalTreeAPI.get_products)
                    if page is None:
                        continue
                    async for page in page.fetch_all():
                        if page is None:
                            continue
                        await writer.writerows(list(map(lambda x: [x.article, x.url], page.items)))

                if data['product']:
                    page = await Pagination.create(node_id, sie, SieportalTreeAPI.get_accesories)
                    if page is None:
                        continue
                    async for page in page.fetch_all():
                        if page is None:
