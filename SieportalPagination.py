...
class Pagination:
    def __init__(self, id: str | int, API: SieportalTreeAPI, product_count: int = -1, func: Callable = SieportalTreeAPI.get_products):
        self.func = func
    ...
    @classmethod
    async def create(cls, id: str | int, API: SieportalTreeAPI, func: Callable = SieportalTreeAPI.get_products) -> Optional['Pagination']:
    ...
    async def select_page(self, page_num: int) -> Any:
        if not (1 <= page_num <= self.max_page):
            raise ValueError(f"Страница {page_num} не существует. Доступные страницы: от 1 до {self.max_page}")
